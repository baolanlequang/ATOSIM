...
#SBATCH --output=logs/atosim-selfish-stubborn-lead-%A_%a.out
#SBATCH --error=logs/atosim-selfish-stubborn-lead-%A_%a.err
...
# own JVM/heap, so the prior 600gb/550G request (needed only because memory
...
java -Xmx48G \
     ...
     sampling/configurations_selfish_stubborn_lead.csv \
