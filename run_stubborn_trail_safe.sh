#!/bin/bash
#SBATCH --job-name=atosim-stubborn-trail-safe
#SBATCH --output=logs/atosim-stubborn-trail-safe-%A_%a.out
#SBATCH --error=logs/atosim-stubborn-trail-safe-%A_%a.err
...
# SAFE-BATCH COPY of run_stubborn_trail.sh -- the original is untouched. See
# run_selfish_safe.sh's header for the full rationale -- identical here,
# just this strategy's CSV/jar/output.
#
# Submit via:
#   ./submit_chunked_array.sh run_stubborn_trail_safe.sh 249800 20 5 0 500
...
TMP_OUT="${TMPDIR}/trail_stubborn_safe_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${TMP_OUT}"
...
for (( ROW_INDEX=ROW_START; ROW_INDEX<=ROW_END; ROW_INDEX++ )); do
    java -Xmx48G \
         ...
         sanity_check_run/campaign_prep/run_configurations_trail_stubborn_safe.csv \
