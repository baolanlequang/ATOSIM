#!/bin/bash
#SBATCH --job-name=atosim-selfish-sanity
#SBATCH --output=logs/atosim-selfish-sanity-%A_%a.out
#SBATCH --error=logs/atosim-selfish-sanity-%A_%a.err
...
# Adapted from ../../run_selfish.sh for the 3000-row (1000/strategy)
...
# --cpus-per-task/--mem/--partition unchanged from run_selfish.sh: a single
# row's simulation still uses up to monteCarloParallelism=8 cores regardless
# of how many total rows are being swept, so per-task resource needs don't
# scale down with the smaller sanity sample -- only wall time and array size
# do (see below).
#
# --time reduced 72:00:00 -> 12:00:00: run_selfish.sh's own comment already
# established ROWS_PER_TASK=500 is "comfortably inside --time=12:00:00" per
# task (500 rows * mean ~19s/row =~2.6h; the 72h figure there is a large
# safety margin sized for the full 250k-row campaign's worst-case tail
# across hundreds of concurrent tasks). Reusing the same ROWS_PER_TASK means
# reusing that same validated per-task time budget directly, just with far
# fewer tasks.
#
...
# (${WORKSPACE_PATH}/selfish etc.) that the real 250k run writes to. Chosen
# over a distinctly-named scratch workspace to avoid requiring a second
# `ws_allocate` call for a 3000-row test -- the existing 40TiB/20M-inode
# workspace has ample room, and this subpath keeps sanity output fully out
# of the real per-strategy directories the analysis pipeline reads from.
...
RESULTS_DIR="${WORKSPACE_PATH}/sanity_check_run/selfish"
...
TMP_OUT="${TMPDIR}/selfish_sanity_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${TMP_OUT}"
