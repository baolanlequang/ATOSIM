#!/bin/bash
#SBATCH --job-name=atosim-stubborn-trail-sanity
#SBATCH --output=logs/atosim-stubborn-trail-sanity-%A_%a.out
#SBATCH --error=logs/atosim-stubborn-trail-sanity-%A_%a.err
...
# Adapted from ../../run_stubborn_trail.sh for the 3000-row (1000/strategy)
...
# --cpus-per-task/--mem/--partition unchanged from run_stubborn_trail.sh: a
...
#
# --time reduced 72:00:00 -> 12:00:00: run_stubborn_trail.sh's own comment
...
# (${WORKSPACE_PATH}/trail_stubborn etc.) that the real 250k run writes to.
...
RESULTS_DIR="${WORKSPACE_PATH}/sanity_check_run/trail_stubborn"
...
TMP_OUT="${TMPDIR}/trail_stubborn_sanity_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${TMP_OUT}"
