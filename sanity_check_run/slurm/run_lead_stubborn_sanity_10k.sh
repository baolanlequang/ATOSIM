#!/bin/bash
#SBATCH --job-name=atosim-stubborn-lead-sanity-10k
#SBATCH --output=logs/atosim-stubborn-lead-sanity-10k-%A_%a.out
#SBATCH --error=logs/atosim-stubborn-lead-sanity-10k-%A_%a.err
...
# Adapted from ../../run_stubborn_lead.sh (via the 1000-row
# run_lead_stubborn_sanity.sh) for the 30,000-row (10,000/strategy)
...
# --cpus-per-task/--mem/--partition unchanged from run_stubborn_lead.sh and
...
# from the live per-strategy dirs (${WORKSPACE_PATH}/lead_stubborn etc.)
...
RESULTS_DIR="${WORKSPACE_PATH}/sanity_check_run_10k/lead_stubborn"
...
TMP_OUT="${TMPDIR}/lead_stubborn_sanity_10k_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${TMP_OUT}"
