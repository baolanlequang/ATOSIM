#!/bin/bash
#SBATCH --job-name=atosim-selfish-sanity-10k
#SBATCH --output=logs/atosim-selfish-sanity-10k-%A_%a.out
#SBATCH --error=logs/atosim-selfish-sanity-10k-%A_%a.err
...
# Adapted from ../../run_selfish.sh (via the 1000-row
# run_selfish_sanity.sh) for the 30,000-row (10,000/strategy) sanity-check
# sample against the regenerated node_degree in [2,8] design -- one scale
# step up from the 3,000-row (1,000/strategy) sanity check, before
...
# --cpus-per-task/--mem/--partition unchanged from run_selfish.sh and the
# 1000-row sanity script: a single row's simulation still uses up to
...
# from the live per-strategy dirs (${WORKSPACE_PATH}/selfish etc.) that the
# real 250k run writes to. Kept distinct from the 1000-row sanity output so
# the two sanity scales don't get mixed when analyzing results later.
...
RESULTS_DIR="${WORKSPACE_PATH}/sanity_check_run_10k/selfish"
...
TMP_OUT="${TMPDIR}/selfish_sanity_10k_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${TMP_OUT}"
