#!/bin/bash
#SBATCH --job-name=atosim-stubborn-trail-hmax600
#SBATCH --output=logs/atosim-stubborn-trail-hmax600-%A_%a.out
#SBATCH --error=logs/atosim-stubborn-trail-hmax600-%A_%a.err
...
# UNIFIED-CAMPAIGN COPY of run_stubborn_trail.sh -- the original is untouched.
...
# unfiltered 250,000-row sampling/run_configurations_trail_stubborn.csv,
# replacing the safe/risky split (run_stubborn_trail_safe.sh +
# run_stubborn_trail_risky.sh) entirely.
#
...
# since run_stubborn_trail_risky.sh still references it).
#
...
# (${WORKSPACE_PATH}/trail_stubborn).
#
# Submit via:
#   ./submit_chunked_array.sh run_stubborn_trail_hmax600.sh 250000 [chunk_size] [sleep_between] [start_offset] [rows_per_task]
# or directly:
#   sbatch run_stubborn_trail_hmax600.sh
...
RESULTS_DIR="${WORKSPACE_PATH}/trail_stubborn"
mkdir -p "${RESULTS_DIR}" logs

TMP_OUT="${TMPDIR}/trail_stubborn_hmax600_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${TMP_OUT}"
