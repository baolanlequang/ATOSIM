...
# run_stubborn_trail.sh` runs, covering the full 250,000-row CSV at
# ROWS_PER_TASK=500. To use a different ROWS_PER_TASK, or to stay under a
# cluster MaxArraySize below 500, submit via:
#   ./submit_chunked_array.sh run_stubborn_trail.sh 250000 [chunk_size] [sleep_between] [start_offset] [rows_per_task]
...
# quota exceeded" writing results_new/trail_stubborn there). bwUniCluster 3.0
...
TMP_OUT="${TMPDIR}/trail_stubborn_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${TMP_OUT}"
